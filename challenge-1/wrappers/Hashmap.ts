...
export class Hashmap implements Contract {
    ...
    async sendClearOldValues(provider: ContractProvider, via: Sender, value: bigint, queryId: bigint) {
        await provider.internal(via, {
            value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
    ...
    async getKey(provider: ContractProvider, key: bigint): Promise<[bigint, Slice]> {
